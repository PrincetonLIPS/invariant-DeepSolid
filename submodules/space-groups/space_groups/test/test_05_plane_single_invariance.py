...
class TestPlaneSingleInvariance(ut.TestCase):
  ...
  def test_01_translation(self, group_num):
    ...
    for ii in range(25):
      ...
      point = jrnd.normal(self.rng_split(), shape=(2,))
      mesh = jnp.stack(
          jnp.meshgrid(*[jnp.arange(-2, 3) for _ in range(2)]),
          axis=-1,
        ).reshape(-1,2) + point[jnp.newaxis,:]
  ...
  def test_02_random(self, group_num):
    ...
    for ii in range(25):
      ...
      points = jrnd.normal(self.rng_split(), shape=(100,2))
  ...
  def test_03_actions(self, group_num):
    ...
    for ii in range(25):
      ...
      pts = jrnd.normal(self.rng_split(), shape=(100,2))
