...
num_args = range(2, 5)
supercell_sizes = [(2,2), (3,3)] # FIXME
test_params = list(product(group_nums, num_args, supercell_sizes))

class TestPlaneDiagonalInvariance(ut.TestCase):
  ...
  def test_01_diagonal_translation(self, group_num, num_args, supercell):
    ...
    for ii in range(num_rounds):
      ...
      point = jrnd.normal(self.rng_split(), shape=(1, num_args, 2,))
      mesh = jnp.stack(
          jnp.meshgrid(*[jnp.arange(-2, 3) for _ in range(2)]),
          axis=-1,
        ).reshape(-1,2)
    ...
    for ii in range(num_rounds):
      for jj in range(num_args):
        ...
        mesh = jnp.stack(
            jnp.meshgrid(*[jnp.arange(-2, 3) for _ in range(2)]),
            axis=-1,
          ).reshape(-1,2)*jnp.array([supercell])
  ...
  def test_03_diagonal_action(self, group_num, num_args, supercell):
    ...
    for ii in range(num_rounds):
      ...
      pts = jrnd.normal(self.rng_split(), shape=(100, num_args,2))
    ...
    for ii in range(num_rounds):
      ...
      for jj in range(num_args):
        ...
        for op in operations:
          ...
          op_pts = pts.at[:,jj,:].set(op_jj_args[...,:2])
